...
open class RgbaDrawer(context: Context): BaseDrawer(context) {
    ...
    override fun onInitParam() {
        mRgbaTextureHandler = GLES30.glGetUniformLocation(mProgram, "sampler")
    }
    ...
    override fun uploadData(textures: IntArray, data: RenderData?) {
        ...
        data?.let {
            GLES30.glTexImage2D(GLES30.GL_TEXTURE_2D, 0, GLES30.GL_RGBA, it.w, it.h,
                0, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE, it.y)
        }
        GLES30.glUniform1i(mRgbaTextureHandler, 0)
    }
}